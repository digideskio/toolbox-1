...
Why did we write `read.csv("data/NPAS-data.csv")` instead of `read.csv(data/NPAS-data.csv)`? Enclosing a string of text into quotation marks in R means that you are referring to a _string_ of text, which is appropriate in this case. If you leave out the quotation marks, R thinks that you are talking about an object, which is not found in the R environment.

As a result of executing `d <- read.csv("data/NPAS-data.csv")`, there is now an _object_ in the R environment, called `d`:
...
Before we begin, make sure that your working directory is set: It should be the `toolbox/import/` folder:
...
pg <- read_csv("data/countries/PG.csv")
pg
```
...
```{r}
( gt <- read_csv("data/countries/GT.csv") )
```
...
```{r, eval = F}
write_csv(d, path = "data/master-data.csv")
```
