...
c_list <- read_tsv("data/data.csv") %>%
    split(., .$country)
names(c_list)
split_write <- function(x) {
    write.csv(c_list[[x]], 
              file = paste0("data/countries/",
                            x, ".csv"))
}
sapply(names(c_list), split_write)
```

# Project: Personality Questionnaire Data
#### `r Sys.Date()`
...
Before we begin wrangling the data, we'll go through an extremely important topic: Project Organization.
...
We will be working with a fairly simple, but I think common, situation where we have multiple "spreadsheet" like data files, and intend to do keep these files on one computer, and produce figures and simple reports from the data. Therefore, we'll set out to define a clear folder structure for our files. A folder structure helps us keep track of our files, ensure that we know what are the inputs (never overwrite the source data!), and how to turn the inputs into outputs. The folder structure I've chosen for this project is displayed below:
...
I view a clear folder structure as the first step toward a reproducible research project. By adhering to a simple scheme like this, my future collaborators--including myself!--will almost immediately know how to locate any files they need to do their jobs.

If you downloaded all of the [Toolbox 2.0](https://mvuorre.github.io/toolbox/) materials, you can now browse your way to these folders on your computer, and inspect their contents. 

# Browsing files and folders in RStudio

Before we can access the data in RStudio, we need to set the working directory. Do __not__ set working directories in your script, because these calls make others with different root folder structures (usernames, systems, etc.) unable to run your script without changing these arguments. Instead, you can simply click the Set As Working Directory button in RStudio as described in the [previous section](http://mvuorre.github.io/toolbox/rrstudio/rrstudio.html).

The raw data downloaded from the [website](http://personality-testing.info/_rawdata/BIG5.zip) is a neat and simple spreadsheet of data, but for our purposes, I've split the data into separate files by country. Here we simply take a look at those files, and in the next section write a little function that merges all those country-specific files into one single file. To look at our source files, we use the `list.files()` function that simply takes a path as input, and shows what's in that path.

```{r}
file_path <- "data/countries/"
...
>The preceding section can be quite difficult if you are only beginning with R. You can skip this step and jump straight into [loading the data](#Load_the_data), and return to this step later when you have more experience with R code.

We have a folder with a bunch of .csv files that we would like to read and save into one master .csv file. Therefore, our task is to write a function that takes a list of files as an input, reads and combines those files, and then saves the output. Because we are only beginning our journey through R, we are not going to invent our own function, but will simply type in a [function that another user has already provided](https://gist.github.com/crsh/357458c41fd3d554fb24) for this specific task

```{r, echo = 3}
address <- "https://gist.githubusercontent.com/crsh/357458c41fd3d554fb24/raw/e0089d976e8768bb7438f0295ffc3834697e6c2a/batch_read.R"
source(address)
batch_read
```

The printout above describes `batch_read()` as a _function_, that takes two arguments: `path` and `extension`. When this function is executed, it first lists files in `path` whose filename include `extension`. This is extremely helpful, because the function now only reads files that have our pre-specified extension, and ignores all other file types. This list of file names is then saved to `data.files`. 

The second line is a little more complicated, but not much! (One reason it looks more complicated than it is has to do with nesting functions vs. piping functions, which we will get to later). The innermost function `paste0` concatenates multiple text strings together, without spaces (hence `paste0` instead of `paste`):
...
Because `path` is a text string, and `data.files` is a list of text strings, the result is a new list of text strings. The result of `paste0(path, data.files)` is then passed to `lapply()`, that literally means _list apply_. `lapply()` takes as its first argument a list, here the list of file names. The second argument to `lapply()` is the function to apply to the list: We want to `read.csv()`. Additionally, this function allows for further arguments, as specified by `...`, but we ignore this for now. This line, therefore, _reads a list of csv files_, and saves the list into `data`.

A final step remains: The list needs to be unpacked into a data frame. `do.call()` takes a function and a list of arguments to the function. The function here is `rbind` (for row-bind; bind data frames together by row), and its argument is the previously constructed list called `data`.

## Write our own function

I said before that programming is hard. You learn it by doing it. The preceding discussion is abstract and elusive and we will learn faster by implementing the function ourselves (with credits to its original author: <https://github.com/crsh>):

```{r}
batch_read <- function(path, extension) {
  data.files <- list.files(path, pattern = extension)
  data <- lapply(paste0(path, data.files), read.csv)
  data <- do.call("rbind", data)
  data
}
```

## Use a function to load and merge multiple data files

Let's put our shiny new `batch_read()` to use and load our data!
...
Now that the data is available in a single .csv file in a local folder, the first step is to load it into the current R environment, or workspace. To load the data, we use a function called `read_tsv()` from the `readr` package [@wickham_readr:_2015]. `read_tsv()` takes _tab-separated_ data tables as input, which is appropriate in this case.
...
## `data.frame`s and `data_frame`s

__Note to readers who already use R__: A data_frame is slightly different, but arguably better, than the base-R data.frame. For instance, note the different printing behavior of `data.frame`s and `data_frame`s. We can, however, conceptually treat it as a data.frame for almost any application.

```{r}
head(read.csv("data/data.csv", sep = "\t"))
```

Compare this with the behavior of a `data_frame`, above. That's why we prefer `data_frame`s.
